...
# pt: Ecc_Point, coeff: int, curve_ins: 曲线参数的库
def ec_mul(pt, coeff, curve_ins):
    if hasattr(coeff, "n"):
        coeff = coeff.n
    return curve_ins.multiply(pt, coeff % curve_ins.curve_order)
...
def ec_lincomb(pairs,curve_ins)-> Point2D:
    return lincomb(
        [pt for (pt, _) in pairs],
        [int(n) % curve_ins.curve_order for (_, n) in pairs],
        curve_ins.add,
        curve_ins.Z1,
    )
    # Equivalent to:
